...
void cartoonifyImage(Mat srcColor, Mat dst, bool sketchMode, bool alienMode, bool evilMode, int debugType)
{
	...
	imwrite("data/medianBlur.jpg", srcGray);
    ...
    if (!evilMode) {
		...
		imwrite("data/edges.jpg", edges);
        threshold(edges, mask, 120, 255, THRESH_BINARY_INV);
		imwrite("data/mask.jpg", mask);
        // Mobile cameras usually have lots of noise, so remove small
        // dots of black noise from the black & white edge mask.
        removePepperNoise(mask);
		imwrite("data/mask_remove_noise.jpg", mask);
    }
    else {
	...
	imwrite("data/blur.jpg", srcColor);
